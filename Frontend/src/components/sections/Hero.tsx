...
export const Hero: React.FC = () => {
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (content) {
        content.style.transform = `translate(${moveX * 0.1}px, ${moveY * 0.1}px)`;
      }
    };
  ...
  return (
    <div 
      ...
      style={{
        backgroundImage: 'radial-gradient(circle at 50% 50%, rgba(15, 86, 179, 0.05) 0%, rgba(255, 255, 255, 0) 70%)',
      ...
      <div className="container max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="hero-content transition-transform duration-200 ease-out max-w-4xl">
          ...
          <h1 className="font-display font-bold text-4xl sm:text-5xl md:text-6xl lg:text-7xl leading-tight mb-6 opacity-0 animate-reveal-1">
            Share Knowledge.<br /> 
            <span className="text-primary">Gain Wisdom.</span>
          </h1>
          
          <p className="text-lg sm:text-xl md:text-2xl text-foreground/80 max-w-2xl mb-8 opacity-0 animate-reveal-2">
            Connect with industry experts for personalized guidance, mentorship, and knowledge sharing in a supportive community.
          </p>
          ...
          <div className="mt-16 grid grid-cols-2 sm:grid-cols-4 gap-8 opacity-0 animate-reveal-4">
            <div className="text-center">
              <div className="font-display font-bold text-3xl sm:text-4xl text-primary">1000+</div>
              <div className="text-sm text-muted-foreground mt-1">Expert Mentors</div>
            </div>
            <div className="text-center">
              <div className="font-display font-bold text-3xl sm:text-4xl text-primary">24/7</div>
              <div className="text-sm text-muted-foreground mt-1">Support Access</div>
            </div>
            <div className="text-center">
              <div className="font-display font-bold text-3xl sm:text-4xl text-primary">50k+</div>
              <div className="text-sm text-muted-foreground mt-1">Active Users</div>
            </div>
            <div className="text-center">
              <div className="font-display font-bold text-3xl sm:text-4xl text-primary">98%</div>
